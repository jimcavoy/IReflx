...
#else
        sprintf(szErr, "Error setting socket option SO_REUSEADDR: %d", errno);
#endif
        std::runtime_error exp(szErr);
        throw exp;
    }
...
#else
        sprintf(szErr, "Error setting socket option SO_BROADCAST: %d", errno);
#endif
        std::runtime_error exp(szErr);
        throw exp;
    }
...
#ifdef _WIN32
        WSACleanup();
        sprintf(szErr, "Failed to bind socket: %d", WSAGetLastError());
        closesocket(_pimpl->_listenSocket);
#else
        sprintf(szErr, "Failed to bind socket: %d", errno);
...
#ifdef _WIN32
            WSACleanup();
            sprintf(szErr, "Unable to open input stream, %s.  Error code: %d\n", _pimpl->_ipmcast.c_str(), dwRetval);
            closesocket(_pimpl->_listenSocket);
#else
            sprintf(szErr, "Unable to open input stream, %s.  Error code: %d\n", _pimpl->_ipmcast.c_str(), dwRetval);
            close(_pimpl->_listenSocket);
